...
- Generación de PDFs individuales o completos
...
│   │   ├── dao/ (PersonajesDAO.java)
│   │   ├── servicios/ (ServicioArchivos.py)
│   │   └── controladores/ (ControladorFichaPersonaje.java, ControladorVisualizarPersonajes.java)
│   └── resources/es/potersitos/
│   │   ├── fxml/ (visualizarPersonajes.fxml, fichaPersonaje.fxml)
│   │   ├── css/ (estilo.css)
│   │   ├── bin/ (todosPersonajes.bin)
│   │   ├── csv/ (todosPersonajes.csv)
│   │   ├── xml/ (todosPersonajes.xml)
...
- 🔍 Filtros Inteligentes → Casa (Gryffindor, Slytherin...), nombre, nacionalidad, género, especie
- 📄 Conversión Múltiple → CSV → XML → Binario
- 🖼️ Imágenes Blobs → Visualización y almacenamiento de fotos
- 📄 Generación PDF → Anuario completo o fichas individuales
...
java -jar target/Reto3_Hogwarts_Anuario-1.0.0.jar
